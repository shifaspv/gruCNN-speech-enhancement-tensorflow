...
def compute_receptive_field_length(dilations): 
    # input length/ receptive field length
    receptive_field= sum(dilations)+1

    return receptive_field


def compute_feature_dim(input_length,target_field_length):
    # compute intermediate layers size/ feature dim
    K=input_length/target_field_length
    layers=np.log(K)/np.log(2)
    feature_dims=np.asarray([2**(i) for i in np.arange(layers+1)])*target_field_length
    feature_dims=np.flip(feature_dims)
